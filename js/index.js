...
$.extend(Action.prototype, {
    ...
    config: {
        ...
        boom: new Image(), //绘制爆炸
        combo: 0, //连击
        validTime: 1500, //combo有效时间2000ms
        lightingSpeed: 0.2, //闪电模式的速度
        lightingModeTimer: 0, //闪电模式时间种子
        isLighting: 0, //闪电数
        lightingShowTime:2000,//闪电出现的时间3000ms
    ...
    canvasTouch: function(ctx, e, self) {
        ...
        for (var i in eMap) {
            ...
            if (x > eX && x < eX + eWidth * trans && y > eY && y < eY + eHeight * trans) {
                ...
                if (eMap[i] == now) return;
                //清除上一个element，并重绘
                var now_x = eMap[i].binaryPoint.x,
                    now_y = eMap[i].binaryPoint.y;
                self.config.currentEMap[now_y][now_x].status = 1;
                var last_x = last.binaryPoint.x,
                    last_y = last.binaryPoint.y;
                if (last_x != -1&&eMap[i].binaryPoint!=last.binaryPoint) {
                ...
                if (last != now && last.mark != -1 && now.mark == last.mark) {
                    if (self.findPath(ctx)) { //是可行的元素绘制路线
                        ...
                        //绘制爆炸
                        setTimeout(function() {
                            self.drawBoom(ctx, last.ePoint, now.ePoint);
                        }, 100);
                    } else { //没有可行路线，清空连击
                    ...
                    self.config.combo = 0; //连击清空
                    last.status = 0;
                    self.config.currentEMap[last.binaryPoint.y][last.binaryPoint.x].status = 0;
                    clearTimeout(self.config.timer);
                    var clientHeight = self.config.clientHeight,
                        clientWidth = self.config.clientWidth,
                        y = 15, //comob位置
    ...
    drawBoom: function(ctx, lastPoint, nowPoint) {
        var self = this;
    ...
    lightingMode: function(ctx) {
        ...
        var time=self.config.lightingShowTime;
        self.config.nowElement=new Element();
        self.config.Element=new Element();
